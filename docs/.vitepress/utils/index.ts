import { isActive, normalize, endingSlashRE, isExternal } from 'vitepress/dist/client/theme-default/utils'
...
export function insertLinkIcon(contentRef: any) {
	if (!inBrowser) return
	const links = Array.from(contentRef.value?.$el.querySelectorAll('a:not(.header-anchor)') ?? [])

	links.forEach((link: any) => {
		link.classList.add('vp-link')
		if (!link.href.startsWith(window.origin) && !link.innerHTML.includes('<img')) {
			link.innerHTML = `
        ${link.innerHTML}
